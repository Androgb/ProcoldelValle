...
let tableVentas = new DataTable('#tablaMovimientos', {
    language: {
		"pageLength": 5,
	  ...
	  if (result.isConfirmed) {
		...
		if (fileName === "") {
			Swal.fire('¡Olvidaste ingresar el nombre del Archivo!', '', 'error')
				...
				let link = '/ventasExcel/' + fileName + '/' + directorySelected + "/";
	  ...
	  if (result.isConfirmed) {
		...
		if (fileName === "") {
			Swal.fire('¡Olvidaste ingresar el nombre del Archivo!', '', 'error')
				...
				//Asigna el valor del enlace
				let link = '/ventasPdf/' + fileName + '/' + directorySelected + "/";
